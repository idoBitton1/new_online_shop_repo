...
//material ui
import { Dialog, DialogContent, DialogTitle, Typography, Button, Checkbox, FormControl, InputLabel, ListItemText, MenuItem, OutlinedInput, Select, SelectChangeEvent, TextField } from "@mui/material";
...
export const AddProductDialog: React.FC<MyProps> = ({is_open, toggleDialog}) => {
    ...
    return(
        <Dialog open={is_open} onClose={closeDialog} fullWidth>
            ...
            <DialogContent>
                <div className={classes.add_product_contex}>
                    <div className="image_selection">
                        <img src={image.image_location} alt={"procduct"} className={classes.product_img} />
                        <input type="file" name="file" onChange={handleImageChange} accept="image/png, image/jpg" />
                        <p className={classes.asterisk_text}>
                            *image will not be saved. to save the image upload it in aws s3, in the link:
                        </p>
                        <a target="_blank" rel="noreferrer"
                        href="https://s3.console.aws.amazon.com/s3/upload/shop-me-online-bucket?region=eu-central-1"
                        className={classes.asterisk_text}>
                            here
                        </a>
                    </div>
